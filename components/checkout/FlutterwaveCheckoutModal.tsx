...
import {
  ...
  parseFlutterwaveContentProbe,
} from '@/lib/flutterwave/checkoutContentProbe';
import { FLUTTERWAVE_CHECKOUT_VIEWPORT_JS } from '@/lib/flutterwave/injectedCheckoutViewport';
import {
  FLUTTERWAVE_CHECKOUT_USER_AGENT,
  isFlutterwaveCheckoutPageUrl,
} from '@/lib/flutterwave/checkoutWebView';
...
export function FlutterwaveCheckoutModal({
  ...
  const scheduleBlankFallback = useCallback(() => {
    ...
    blankTimerRef.current = setTimeout(() => {
      if (completedRef.current || contentReadyRef.current) return;
  ...
  useEffect(() => {
    ...
    contentProbeTimerRef.current = setTimeout(() => {
      if (!contentReadyRef.current && !completedRef.current) {
        scheduleBlankFallback();
      }
  ...
  }, [visible, url, scheduleBlankFallback, clearBlankTimer, clearContentProbeTimer]);
